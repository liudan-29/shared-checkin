...
├── components/           共享组件（HistoryTag/MissedMark/SummarySheet等只读/预览/总结相关；GoalRow/GoalEditorSheet/WeeklyCompareTable/ShareCard等PDCA周报相关；MonthGrid/DateJumpSheet日历选择器；MessageBoard/MessageComposerDialog/MessageHistoryDialog留言板）
├── lib/
│   ├── supabase.ts       Supabase 客户端
│   ├── types.ts          共享类型（Slot、PlanSlot、DayPlan、Template、CycleGoal、WeeklyReview、Message）
...
│   ├── messages.ts       messages表的CRUD（限量拉取最近留言、发布、Realtime订阅新留言）
