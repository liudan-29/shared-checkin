...
## 2026-07-16 08:53（使用手册同步更新）

用户要求更新产品使用手册。检查发现`docs/user-guide.html`本来就没写过PDCA周期目标那部分内容（手册最早成稿在PDCA功能之前，之后没回头补过），所以这次不涉及删除，只需要新增留言板和模板同步到今天这两块：

- 新增"07 留言板"整节：写留言（配一个飘动横条的静态小样）、查看历史/删除（配一个列表小样）、收起留言板；"03 打卡"节里产出记录的说明补了一条"保存后会问要不要发到留言板"
- "08 日常安排"（原07模板设置）补了"同步到今天"的说明卡片
...
- GitHub Pages工作流`36083856108`执行成功；线上`/login/`返回HTTP 200，页面标题为“三人打卡”，文件SHA-256与本地本次构建完全一致，确认CDN已刷新到三人版
- 待完成：三个真实账号的登录、三栏展示和Realtime同步仍需三位用户在各自设备上验收
